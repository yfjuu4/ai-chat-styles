...
// @version      3.2
// @description  Load custom CSS for ChatGPT, Claude AI, and Google Search via jsDelivr - Berry Browser Optimized
...
const CONFIG = {
    ...
    GOOGLE_DELAY: 2000 // Extra delay for Google's dynamic loading
};
...
const utils = {
    log(message, level = 'info') {
        ...
        const siteIcon = state.site.isGoogle ? '🔍' : '🎨';
        ...
        if (state.site.isGoogle) {
            ...
            for (const selector of state.site.searchSelectors) {
                ...
                if (element) {
                    ...
                    // Extra delay for Berry Browser on Google
                    if (state.isBerryBrowser) {
...
// 🎨 Style manager (keep as is, no changes needed)
const styleManager = {
    async apply() {
...
const uiManager = {
    ...
    updateButtonState(button) {
        ...
        // Special icons for different sites
        let icon;
        if (state.site.isGoogle) {
            icon = state.enabled ? '🔍' : '🔎';
        } else {
...
const app = {
    async init() {
        utils.log(`🚀 Initializing ${state.site.name} Styler v3.2`, 'info');
        utils.log(`Mode: ${state.isBerryBrowser ? '🍓 Berry Browser' : 'Standard'}`, 'info');
   
        // Add CSS animations
        this.addPulseAnimation();
   
        // Initial delay - longer for Google on Berry Browser
